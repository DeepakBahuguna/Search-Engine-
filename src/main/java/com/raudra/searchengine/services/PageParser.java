...
public class PageParser {
    ...
    public void parse(WikiPage page) throws IOException {
      ...
      //  Map<String,Integer[]> wordCount=new HashMap<String,Inetger[]>(256);
...
/*    private int maxFreq(Map<String,Integer[]> freq){
    ...
    throws IOException
    {
        ...
        while(entries.hasNext()){
            ...
            if(doclist==null)
            {
                ...
                doclist.append(page.getId()).append(WikiPageParsingConstants.DOC_COUNT_DELIMITER).append(getFieldsString(entry.getValue()).append(WikiPageParsingConstant.DOC_COUNT_DELIMITER));
                allWords.put(entry.getKey(),doclist);
            }
            else{
                doclist.append(page.getId()).append(WikiPageParsingConstants.DOC_COUNT_DELIMITER).append(getFieldsString(entry.getValue()).append(WikiPgeParsingConstants.CHAR_DOC_DELIMITER));
            }
        }
    }
    public void insertToAllWords(WikiPage page, Map<String,Integer[]> wordCount)
    ...
    {
        String []=text.split(WikiPageParsingConstants.DOC_PARSIGN_REGEX);
       // String []=text.split(WikiPageParsingCOnstants.DOC_PARSIGN_REGEX);
            Integer[] count;
...
/*    public void printLineToIndexFile(List<MergeLine> sameWords, String currentWord,
    ...
    private StringBuilder getFieldsString(Integer[] values)
    {
        ...
        valueString.append(setBit).append(WikiPageParsingConstants.WEIGHT_DELIMITER).append( (termFrequence(weight)));
        return valueString;
    }
    private StringBuilder getFieldsString(Integer[] values){
